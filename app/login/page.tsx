...
function LoginForm() {
  ...
  return (
    <main className="w-full max-w-md mx-4 p-10 rounded-xl bg-surface-container-low/40 backdrop-blur-[20px] border border-primary-container/15 relative z-10 flex flex-col gap-8 shadow-[0_0_60px_rgba(0,229,255,0.03)] animate-[fadeIn_0.5s_ease-out]">
      {/* Header Section */}
      ...
      {isRegistered && (
        <div className="px-4 py-3 rounded-lg border border-primary-container/40 bg-primary-container/10 text-primary-container text-xs font-mono flex items-center gap-2 animate-[fadeIn_0.3s_ease-out]">
          <span className="material-symbols-outlined text-base">verified</span>
          <span>Player registered successfully! Please authenticate below.</span>
        </div>
      )}
...
export default function LoginPage() {
  return (
    <div className="bg-background text-on-background min-h-screen flex flex-col font-body-md text-body-md relative overflow-hidden selection:bg-primary-container/30 selection:text-primary-fixed">
      <Navbar />
      ...
      <div className="flex-1 flex items-center justify-center py-12 relative z-10">
        <Suspense fallback={<div className="text-on-surface-variant text-xs font-mono">Loading form...</div>}>
          <LoginForm />
        </Suspense>
      </div>
